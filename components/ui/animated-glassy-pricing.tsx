  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main(){
        ...
        color=mix(color,vec3(1.),paintCircle(uv,center,radius,.003).r * 0.3);
...
export const PricingCard = ({
  planName, description, price, priceSubtext = "/mo", features, buttonText, buttonHref = "/contact", isPopular = false, buttonVariant = 'primary'
}: PricingCardProps) => {
  const cardClasses = `
    ...
    ${isPopular ? 'scale-105 relative ring-2 ring-foreground/20 dark:from-white/20 dark:to-white/10 dark:border-foreground/30 shadow-2xl' : ''}
  `;
  const buttonClasses = `
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-foreground text-white">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        <h2 className="text-[32px] font-bold tracking-[-0.02em] text-foreground">{planName}</h2>
        <p className="text-[14px] text-foreground/70 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-6 flex items-baseline gap-2">
        <span className="text-[48px] font-bold text-foreground">{price}</span>
        <span className="text-[14px] text-foreground/70 font-sans">{priceSubtext}</span>
      </div>
      ...
      <ul className="flex flex-col gap-2 text-[14px] text-foreground/90 mb-6 font-sans">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <CheckIcon className="text-foreground w-4 h-4 flex-shrink-0" /> {feature}
          </li>
        ))}
      </ul>
    ...
    <div className="bg-background text-foreground min-h-screen w-full overflow-x-hidden">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          <h1 className="text-[48px] md:text-[64px] font-bold leading-tight tracking-[-0.03em] text-foreground">
            {title}
          </h1>
